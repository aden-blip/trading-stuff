# trading-stuff

Level-to-Level (L2L): a key-level bounce-and-break strategy for NQ/MNQ futures (and other
symbols), written for TradingView Pine Script v6.

Status: **planning**. No code yet.

- `docs/PLAN.md` is the design: what gets built, module by module, with defaults and milestones.
- `docs/DECISIONS.md` is the list of open questions, each with a recommended default and its current status.
- `docs/RESEARCH.md` holds the platform, broker, cost and source-strategy research with links.
- `docs/BACKTEST_LOG.md` records every backtest session: settings, stats, issues, and changes.
- `CLAUDE.md` carries the rules every Claude session follows in this repo.

Reference links and material live in `reference/`. Once code starts, the script lives in `pine/` and any offline
statistics work in `research/`.
